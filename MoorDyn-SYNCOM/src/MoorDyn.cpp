...
void RHSmaster( const double X[],  double Xd[], const double t, double dt)
{
...
{
	RHSmaster(x0, f0, *t0, 0.5*dt);	 								// get derivatives at t0.      f0 = f ( t0, x0 );

	for (int i=0; i<nX; i++) 
		xt[i] = x0[i] + 0.5*dt*f0[i];  						// integrate to t0  + dt/2.        x1 = x0 + dt*f0/2.0;
	
	RHSmaster(xt, f1, *t0 + 0.5*dt, 0.5*dt);							// get derivatives at t0  + dt/2.	f1 = f ( t1, x1 );
			...
			{	
				...
				{ 	
					...
					if (entries.size() >= 7) // if valid number of inputs
					{
						...
						(LinePropList.back()).viscoE = atoi(entries[5].c_str()); 
						(LinePropList.back()).Can= atof(entries[6].c_str());
						(LinePropList.back()).Cat= atof(entries[7].c_str());
						(LinePropList.back()).Cdn= atof(entries[8].c_str());
						(LinePropList.back()).Cdt= atof(entries[9].c_str());
...
//		rdFairi[ii].resize(3);
	...
	for (int l=0; l < nLines; l++) 
	{
		...
		//---------------SYNCOM Modification------------------------//
		LineList[l].SC_offInit();
		LineList[l].SC_updateParams(dtM);
		//---------------End of Modification------------------------//
	}
...
int DECLDIR GetNodePos_v2(int LineNum, int NodeNum, double pos[3])
{
	// output LINE fairlead (top end) tensions
	if ((LineNum > 0) && (LineNum <= nLines)) {
		int worked = LineList[LineNum-1].getNodePos_v2(NodeNum, pos);	// call line member function to fill in coordinates of the node of interest 
		if (worked >= 0)
			return 0;  // success
	}
	return -1;		// otherwise indicate error (invalid node and line number comination)
...
//===============================================================================
//------------------------- SYNCOM Modifications---------------------------------
int DECLDIR SC_GetEpsFL(int LineNum, double eps[])
{
	// output LINE fairlead (top end) stress and deformations
	if ((LineNum > 0) && (LineNum <= nLines)) {
		LineList[LineNum - 1].SC_getEpsFL(eps);	// call line member function to fill in coordinates of the node of interest 
